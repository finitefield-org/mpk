...
def definitions() -> dict:
    ...
    return {
        ...
        "non_templates": [
            {"id": "mpk.csharp.value.unit.v1", "name": "unit", "representation": "zero bit tree", "default": "unit", "operations": ["make", "equal", "compare"]},
            ...
            {"id": "mpk.csharp.value.exception.v1", "name": "exception", "representation": "closed tag plus active payload bit tree",
        ...
        "ordinary_core": {
            "schema": "mpk.csharp.foundation_expansion.v1", "value_carrier": "C(0)=Pi(Nat,Bool); C(d+1)=Pi(Nat,C(d))",
            "zero": "Z(0)=lambda(i:Nat).false; Z(d+1)=lambda(i:Nat).Z(d)",
            "leaf": "fixed-width little-endian bits; out-of-width indexes false",
            "product": "lambda(i).if i=0 then lift(field0) else ... else Z; field projection applies concrete index then unlift",
            "sum": "product(tag, active_payload); zero inactive branches; validity tests only the active payload",
            "sequence": "product(u32 length, lambda(i).if i<length then lift(element[i]) else Z)",
            "construction": "pre-VIR unique-token, bitmap and sequence state; not a storable source value; token erased only after VCs",
            "lift": "lift(x)=lambda(i).if i=0 then x else Z; unlift(x)=x(0); iterate to common child depth",
            "operations": "closed equations in templates and specification sections 4-9; expand conditionals with Bool.rec, bounded folds with Nat.rec, applications/lambdas/Pi/let only",
