...
repo=Path('/Users/kazuyoshitoshiya/mpk');root=repo/'develop/migrations/csharp-03/ordinary-foundation';e=root/'verification-logs/binding-defaults';e.mkdir(parents=True,exist_ok=True)
...
record=dict(status='running',supervisor_pid=os.getpid(),profile=profile,runs=[],full_gate='deferred_to_T06_W12',selection_reason={'build':'Compile original W06 closed actual-default conditions and their integration test owner; keep the checked optimized profile.','definitions':'All 45 original source contexts: exact W06 default sequents and unresolved obligations, strict import mutations, transitive default/projection/guard/public-domain preservation, both checkers and corruption rejection.','semantics':'Every original available actual default, independently evaluated public membership, both conjuncts of every closed condition, and one-bit negative probes for valid source/wrong arm and invalid source/matching arm.','preservation':'Rebuild all 53 source-invariant certificate and metadata pins under current exports; the unchanged bytes preserve the sound applicability of the independent running semantic test.','quality':'Clippy for changed library/integration tests, five artifact inventory tests, and format checks. No full T gate.'}[mode])
...
try:
 if mode=='build':
  ...
  for value in paths:
   binary=Path(value);kind='integration' if binary.name.startswith('csharp_practical_vc-') else 'unit';digest=sha(binary);snapshot=Path('/tmp/mpk-w09-binding-defaults-'+kind+'-'+digest[:16]);shutil.copy2(binary,snapshot);record['binaries'][kind]=dict(path=str(snapshot),sha256=digest)
  ...
  if mode=='definitions':
   env['MPK_W09_BINDING_DEFAULTS_OUT']=str(root/'binding-defaults');test('definitions','csharp_03_t06_w09_binding_defaults_original_source_certificates');manifest=json.loads((root/'binding-defaults/certificates.json').read_text());record['certificate_corpus']={r['id']:r['metadata']['certificate_sha256']for r in manifest['sources']};save();name='TestCheckerAgreementWithRustCLIBindingDefaults';run('checkers',['go','test','-tags=checkeragreement','-count=1','-timeout=0','-run','^'+name+'$','-v','.'],['--- PASS: '+name+'/'+r['id']+'.hex ' for r in manifest['sources']],repo/'go-tools/mpk-checker-ref')
  elif mode=='semantics':
   test('closed-conditions','csharp_03_t06_w09_binding_defaults_original_closed_conditions')
