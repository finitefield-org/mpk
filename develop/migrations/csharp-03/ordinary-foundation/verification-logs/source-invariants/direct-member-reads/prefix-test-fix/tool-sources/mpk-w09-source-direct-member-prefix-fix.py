...
repo=Path('/Users/kazuyoshitoshiya/mpk');root=repo/'develop/migrations/csharp-03/ordinary-foundation';e=root/'verification-logs/source-invariants/direct-member-reads/prefix-test-fix';e.mkdir(parents=True,exist_ok=True)
...
try:
 if mode=='build':
  ...
  for value in paths:
   binary=Path(value);kind='integration' if binary.name.startswith('csharp_practical_vc-') else 'unit';digest=sha(binary);snapshot=Path('/tmp/mpk-w09-source-direct-members-prefix-fix-'+kind+'-'+digest[:16]);shutil.copy2(binary,snapshot);record['binaries'][kind]=dict(path=str(snapshot),sha256=digest)
