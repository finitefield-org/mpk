...
fn same_direct_member(
    ...
    for arg in &prefix {
        let TermNode::Const { global, ref levels } = old.term_table[*arg as usize] else {
            panic!("nonconstant field prefix")
        };
        assert!(levels.is_empty());
        let name = &old.name_table[old.declarations[global as usize].name as usize];
        // Evaluate the closed prefix leaf, never a supplied field result.
        bit(run(old, name, vec![]));
    }
