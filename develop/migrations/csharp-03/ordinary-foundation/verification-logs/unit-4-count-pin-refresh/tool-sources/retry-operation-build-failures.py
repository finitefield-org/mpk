...
repo=Path('/Users/kazuyoshitoshiya/mpk');root=repo/'develop/migrations/csharp-03/ordinary-foundation';e=root/'verification-logs/unit-4-count-pin-refresh';p=e/'transient-operation-build-retries.json';lock=threading.Lock()
def sha(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
assert not p.exists()
r=dict(status='running',supervisor_pid=os.getpid(),cause='Eight existing corpus checks invoked cargo run during the new operation sequent lowerer usize/u32 index error. The library check passed after correcting the free-index arithmetic. Retain every failed log and rerun full original Go/Rust acceptance, identical report comparison and corruption rejection for these byte sets.',full_gate='deferred_to_T06_W12',runs=[])
for batch in ['consumers-batch-1','consumers-batch-2']:
 ...
 for previous in d['runs']:
  ...
  log=e/previous['log'];assert sha(log)==previous['log_sha256'];
  if 'has no field named `value_id`' in log.read_text():continue
  assert 'error[E0308]' in log.read_text() and 'csharp_practical_ordinary_binding_conditions.rs' in log.read_text()
...
assert len(r['runs'])==8
r['fixed_source_sha256']={x:sha(repo/x) for x in ['crates/mpk-vc/src/csharp_practical_ordinary_binding_conditions.rs','crates/mpk-vc/src/csharp_practical_ordinary_concrete_operations.rs']}
def save():
...
def one(row):
 check(row);log=e/('operation-build-retry-'+row['representative'].replace('/','-')+'.log');env=os.environ.copy();env['GOCACHE']='/tmp/mpk-w09-unit4-go-cache';start=time.monotonic()
