...
retry_paths=[retry_path,e/'transient-operation-build-retries.json']
retry_documents=[read(p)for p in retry_paths]
assert all(d['status']=='passed'for d in retry_documents)
for file,h in retry_documents[1]['fixed_source_sha256'].items():assert sha(e/'operation-build-fixed-sources'/Path(file).name)==h
...
for name,d in batches.items():
 ...
 for run in d['runs']:
  if run['status']=='passed':
   ...
   path=e/run['log'];assert sha(path)==run['log_sha256'];assert 'has no field named `value_id`'in path.read_text() or ('error[E0308]'in path.read_text() and 'csharp_practical_ordinary_binding_conditions.rs'in path.read_text())
   key=run['corpus']+'/'+run['selected_files'][0];assert key not in failed;failed[key]=(name,run)
assert len(failed)==11 and [len(d['runs'])for d in retry_documents]==[3,8]
for retry_path,run in [(p,x)for p,d in zip(retry_paths,retry_documents)for x in d['runs']]:
...
result=dict(status='passed',scope='Terminal identical-byte Go/Rust acceptance, identical reports, zero axioms and full hash-corruption rejection for all additional count consumers. Earlier interruptions and eleven transient Rust compilation failures remain retained.',distinct_byte_sequences=224,changed_occurrences=274,full_retries=11,original_batch_status={k:v['status']for k,v in batches.items()},evidence={name:sha(e/name)for name in [*batches,*(p.name for p in retry_paths)]},byte_sets=rows,full_gate='deferred_to_T06_W12')
p=e/'additional-consumers/checker-verification.json';p.write_text(json.dumps(result,indent=2)+'\n')
p=e/'additional-consumers/verification.json';d=read(p);d['status']='regeneration_audit_promotion_and_checkers_passed';d['terminal_checker_verification']='checker-verification.json';d['checker_note']='All 224 current distinct byte sets (274 occurrences) passed unchanged Go/Rust acceptance and hash-corruption rejection. Eleven original Cargo failures are retained and covered by complete terminal reruns; original batch statuses are not rewritten.';p.write_text(json.dumps(d,indent=2)+'\n')
