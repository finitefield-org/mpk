"""Run only the outstanding nullable-reference runtime partitions; completed checks are excluded."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    for path, expected in manifest.get("fixture_sha256", {}).items():
        if digest(repo / path) != expected:
            raise SystemExit(f"Fixture changed since handoff: {path}")
    ...
    if args.output:
        ...
        output = Path(tempfile.mkdtemp(prefix="mpk-w09-reference-data-long-", dir="/tmp"))
    ...
    for row in runtime_cases:
        ...
        env.update(row.get("environment", {}))
        print(f"START {label}", flush=True)
        start = time.monotonic()
        with log.open("w") as stream:
        ...
        passed = (result.returncode == 0 and row["test"] + " ..." in text
                  ...
                  and row["expected_summary"] in text)
        receipt = {**row, "command": command, "exit_code": result.returncode,
                   "passed": passed, "wall_seconds": time.monotonic() - start,
                   "environment": {"MPK_CORE_MAX_SECONDS": row["evaluator_seconds"],
                                   "MPK_CORE_MAX_STEPS": None, "MPK_W09_variables": "unset before selected environment",
                                   **row.get("environment", {})},
                   "log": log.name, "log_sha256": digest(log)}
        results.append(receipt)
        (output / "results.json").write_text(json.dumps(results, indent=2) + "\n")
