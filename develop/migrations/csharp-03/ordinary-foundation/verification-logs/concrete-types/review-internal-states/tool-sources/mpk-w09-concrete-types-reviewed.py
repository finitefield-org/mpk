...
repo=Path('/Users/kazuyoshitoshiya/mpk');root=repo/'develop/migrations/csharp-03/ordinary-foundation';e=root/'verification-logs/concrete-types/review-internal-states';e.mkdir(parents=True,exist_ok=True)
...
record=dict(status='running',supervisor_pid=os.getpid(),profile=profile,runs=[],full_gate='deferred_to_T06_W12',selection_reason={'build':'Compile exact closed type predicate links and original W06 concrete_type_equivalence conditions plus their targeted test owner.','definitions':'All 45 original contexts, 81 public type sequents and six explicitly pending internal-state sequents: exact complete instance/recipe preservation, strict import/context/field/hash/limit mutations, complete independent public-domain closure, and both unchanged checkers with hash-corruption rejection.','semantics':'Original type predicates over all retained concrete instances: independently preserved recursive domains, admitted and invalid representations, and conditions becoming false when only the concrete predicate is changed. No limits or precomputed truths.','preservation':'Rebuild all 53 source-invariant certificate and metadata pins under current exports; the source-invariant scoped verification stays valid for identical bytes.','quality':'Clippy for changed library/integration tests, five artifact inventory tests, and format checks. No full T gate.'}[mode])
...
try:
 if mode=='build':
  ...
  for value in paths:
   binary=Path(value);kind='integration' if binary.name.startswith('csharp_practical_vc-') else 'unit';digest=sha(binary);snapshot=Path('/tmp/mpk-w09-concrete-types-reviewed-'+kind+'-'+digest[:16]);shutil.copy2(binary,snapshot);record['binaries'][kind]=dict(path=str(snapshot),sha256=digest)
