...
fn preserves_existing_declarations(previous_folder: &str, strip_phis: bool) {
    ...
    for file in fs::read_dir(&previous).unwrap() {
        ...
        for function in after_meta["functions"].as_array_mut().unwrap() {
            function.as_object_mut().unwrap().remove("node_entries");
            if let Some(mut memory) = function.as_object_mut().unwrap().remove("memory_bindings") {
                ...
                if matches!(
                    previous_folder,
                    "before-public-projection" | "before-builtin-throw" | "before-node-entry"
                ) {
                    function
                        .as_object_mut()
                        .unwrap()
    ...
    for f in p.functions() {
        ...
        for (entry, block) in f.node_entries.iter().zip(&native.blocks) {
            ...
            let definition = entry
                .definition
                .as_ref()
                .expect("all captured incoming guards are defined");
            ...
            assert!(!bit(run(c, definition, base.clone())));
            observations += 1;
            ...
            for (incoming, edge) in entry.incoming.iter().zip(edges) {
                ...
                for sample in [0, 1, i32::MIN, i32::MAX, -1] {
                    ...
                    assert_eq!(
                        bit(run(c, definition, args.clone())),
                        expected,
                ...
                if let Some(args) = positive {
                    ...
                    assert!(bit(run(c, definition, varied)));
                    observations += 1;
                    for i in 0..n {
            ...
            if positives.len() >= 2 {
                ...
                assert!(!bit(run(c, definition, args)));
                observations += 1;
            }
        }
    }
...
fn run_cases(ids: &[&str], runtime: bool) {
    run_selected_cases(ids, runtime, false);
}
fn run_selected_cases(ids: &[&str], runtime: bool, entries: bool) {
    let bundle = b();
    ...
    for &id in ids {
        ...
        if entries {
            let count = run_node_entries(&p, vir, &c, &depths);
        ...
        for f in p.functions() {
            ...
            for binding in &f.memory_bindings {
                ...
                assert!(!binding.public_slot_projection_pending);
                assert!(binding.public_projection_definedness.is_some());
                assert!(binding.public_projection_definition.is_some());
                assert!(binding.source_slot_snapshot_definition.is_some());
