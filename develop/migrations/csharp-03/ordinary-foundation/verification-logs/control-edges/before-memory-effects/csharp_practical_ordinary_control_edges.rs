...
pub struct OrdinaryControlNodeEntry {
    ...
    pub state_rule: String,
    /// Missing incoming guards leave the complete node relation pending.
...
fn node_entry(
    ...
    let edges = flow
        ...
        .collect::<Vec<_>>();
    for edge in &edges {
        // Bound expansion before cloning any incoming snapshot metadata.
        arguments
            .len()
            .checked_add(1)
            .and_then(|n| n.checked_add(edge.source.guard.bindings.len()))
