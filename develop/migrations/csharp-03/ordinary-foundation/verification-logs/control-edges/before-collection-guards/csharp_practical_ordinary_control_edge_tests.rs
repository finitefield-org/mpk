...
fn run_cases(ids: &[&str], runtime: bool) {
    ...
    for &id in ids {
        ...
        if !runtime {
            ...
            eprintln!("control edges {id}: exact predecessor bytes preserved");
            continue;
        }
        ...
        for f in p.functions() {
            ...
            for edge in &f.edges {
                ...
                for sample in 0..if id == "index_update" { 13 } else { 7 } {
                    let inputs = (0..edge.source.guard.bindings.len())
                        .map(|i| match sample {
                            ...
                            7..=10 => {
                                if p.construction_definitions().iter().any(|d| {
                                    d.construction.carrier.type_id
                                        == edge.source.guard.bindings[i].type_id
                                }) {
                                    2
                                } else {
                                    [0, 1, 2, -1][sample - 7]
                    ...
                    let values = edge
                        ...
                        .map(|(b, n)| {
                            ...
                            if p.construction_definitions()
                                .iter()
                                .any(|d| d.construction.carrier.type_id == b.type_id)
                            {
                                sparse_cube(
                                    depth,
                                    (0..32)
...
#[test]
fn csharp_03_t06_w09_control_edge_missing_string_guards_stay_pending() {
    let bundle = b();
    ...
    assert!(
        ...
        "the source must emit actual W04 control edges"
    );
    assert!(p.construction_definitions().is_empty());
    assert!(p.symbolic_ownership().is_empty());
    let mut pending = 0;
    for e in p
        .functions()
        .iter()
    ...
    assert!(
        pending > 0,
        "missing foreach-string checks must not disappear"
    );
    ...
    .is_err());
    eprintln!("unsupported foreach-string: {pending} explicit pending guards");
}
