...
A separate node-entry relation now selects exactly one incoming edge, requires
...
A successful array rewrite now connects one original receiver slot to exact
...
returns the new array. Other source frames and exceptional execution remain
open.
...
The current 17-source corpus has 704 guards, 667 slot transports, 62 native phi
joins (100 values), and 617 node-entry relations. There are 38,950 retained edge
observations, 13,576 entry observations and 13 successful-update observations,
totaling 52,539. Original W04
binding comparisons cover 95 identities in the initial partition and 36 in the
collection partition. The type fixture has no such original goal bindings;
its storage tests do not prove logical observation mapping.

All 14 distinct certificate byte sequences covering the 17 sources pass both
...
and [memory-effect review](../verification-logs/control-edges/memory-effects/review.md).
...
The larger `count_fill` context requires factoring an eleven-predecessor entry
within the binder limit; no fill-loop entry/effect acceptance is claimed yet.
