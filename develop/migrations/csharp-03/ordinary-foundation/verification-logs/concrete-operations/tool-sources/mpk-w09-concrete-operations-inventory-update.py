...
paths=sorted(set(paths));new={'crates/mpk-vc/tests/support/csharp_practical_ordinary_concrete_operation_tests.rs','crates/mpk-vc/src/csharp_practical_ordinary_concrete_operations.rs'};assert new <= set(paths)
old=[x for x in paths if x not in new];hashpaths=lambda xs:hashlib.sha256(('\n'.join(xs)+'\n').encode()).hexdigest()
...
p=Path('crates/mpk-vc/tests/csharp_practical_inventory.rs');assert t.count('4_968,')==1;p.write_text(t.replace('4_968,','4_970,'))
print('Exact previous145-path preimage preserved;added only',new,';namespace paths',len(paths),'total4970')
