...
failed_isolated=read(earlier/'isolated.json');assert failed_isolated['status']=='failed';row=failed_isolated['runs'][0];assert row['exit_code']!=0 and sha(earlier/row['log'])==row['log_sha256'];assert 'op.normal_agreement_definition'in(earlier/row['log']).read_text()
