        ...
        if id == "float-make-commutation" {
            ...
            *previous = value;
            ...
            let domain = p
                ...
                .unwrap();
            // Some(-1) with nonzero inactive/padding storage violates the exact
            // public Option representation, while the selected payload is -1.
            let args = vec![V::UniformCube(true, domain.carrier.depth)];
            assert!(!bit(run(&cert, &domain.valid_definition, args.clone())));
            let mut changed = cert.clone();
            constant_result(&mut changed, &op.concrete_definition, 1, false);
            assert!(!bit(run(
                &changed,
                &op.normal_agreement_definition,
                args.clone()
