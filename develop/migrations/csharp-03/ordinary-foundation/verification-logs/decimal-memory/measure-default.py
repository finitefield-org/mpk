...
# Set the release executable printed by cargo test --no-run.
binary=os.environ['MPK_W09_MEMORY_BINARY']
root=pathlib.Path(os.environ.get('MPK_W09_MEMORY_OUTPUT','/tmp/mpk-w09-decimal-memory-default'));root.mkdir(exist_ok=True)
profiles={
...
for label in sys.argv[1:]:
 ...
 env=os.environ.copy()
 for key in list(env):
  if key.startswith('MPK_W09_DECIMAL_') or key == 'MPK_W09_CORE_CACHE_POLICY': del env[key]
