...
#[test]
fn active_go_envelopes_use_the_successor_protocol_family() {
    let registry = semantic_registry();
    let index = json(format!("{ACTIVE_ROOT}/frontend-index.json"));
    ...
    for case in index["cases"].as_array().expect("positive cases") {
        ...
        let envelope_bytes = read(Path::new(ACTIVE_ROOT).join(envelope_path));
        let envelope: Value =
