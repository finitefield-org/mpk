//! W03 option foundation operation relations at original SSA use points.
...
pub struct OrdinaryOptionDataDefinition {
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize)]
pub struct OrdinaryOptionDataOperation {
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize)]
pub struct OrdinaryOptionDataProgram {
    schema: String,
    source_ir_sha256: String,
    foundation_sha256: String,
    data_vc_sha256: String,
    definitions: Vec<OrdinaryOptionDataDefinition>,
    operations: Vec<OrdinaryOptionDataOperation>,
...
impl OrdinaryOptionDataProgram {
    pub fn definitions(&self) -> &[OrdinaryOptionDataDefinition] {
        &self.definitions
    }
    pub fn operations(&self) -> &[OrdinaryOptionDataOperation] {
        &self.operations
    }
    ...
    pub fn canonical_bytes(&self) -> Vec<u8> {
        serde_json::to_vec(self).expect("ordinary option data relations")
    }
}
fn name(kind: &str, identity: &impl Serialize) -> String {
    format!(
        "{PREFIX}.OptionData.{kind}.H{:x}",
        Sha256::digest(serde_json::to_vec(identity).expect("typed option data identity"))
    )
}
pub(super) fn emit(
    c: &mut Clauses<'_>,
...
) -> R<OrdinaryOptionDataDefinition> {
    ...
    Ok(OrdinaryOptionDataDefinition {
        source: d.clone(),
...
pub fn generate_csharp_practical_ordinary_option_data(
    vir: &ValidatedPracticalVir,
) -> R<OrdinaryOptionDataProgram> {
        ...
        .filter(|o| ids.contains(o.definition_id.as_str()))
    {
        ...
        operations.push(OrdinaryOptionDataOperation {
            source: o.clone(),
            predicates,
        });
    }
    let certificate = c.b.finish()?;
    let p = OrdinaryOptionDataProgram {
        schema: "mpk.csharp.ordinary_option_data.v1".into(),
...
pub fn import_csharp_practical_ordinary_option_data(
    input: &[u8],
    certificate: &[u8],
    vir: &ValidatedPracticalVir,
) -> R<OrdinaryOptionDataProgram> {
