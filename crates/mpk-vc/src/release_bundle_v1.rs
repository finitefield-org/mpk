//! Active release-bundle models and validators.
//!
//! The resolver accepts only `mpk.release.bundle_registry.v1`. Candidate
//! identifiers retain their reviewed `candidate.v1` and `staging` suffixes as
//! immutable release identity strings; they are not runtime activation flags.
...
const RUST_FRONTEND_VERSION: &str = "0.1.0-profile-v1-staging";
...
impl ValidatedSuccessorReleaseRegistry {
    ...
    pub fn release_identity(&self) -> crate::ReleaseRegistryIdentity {
        crate::ReleaseRegistryIdentity {
...
fn parse_transport(
    input: &[u8],
) -> Result<(crate::StrictJsonValue, Vec<u8>), SuccessorReleaseError> {
...
fn validate_projection(
    ...
    for tuple in tuples {
        ...
        if context.semantic_profile() == "mpk.go.fixed.v0" {
            if frontend.bundle_id != GO_FRONTEND_BUNDLE_ID {
        ...
        if context.semantic_profile() == "mpk.rust.checked.v0" {
            if frontend.bundle_id != RUST_FRONTEND_BUNDLE_ID {
                return Err(linkage_failure());
            }
...
fn validate_frontend(
    ...
    if frontend.bundle_id == GO_FRONTEND_BUNDLE_ID
        && (frontend.name != "go2vir"
            ...
            || frontend.main.binary_sha256 != GO_FRONTEND_SHA256
            || !matches!(frontend.main.runtime, ExecutableRuntime::Static)
    ...
    if frontend.bundle_id == RUST_FRONTEND_BUNDLE_ID {
        ...
        if frontend.name != "rust2vir"
            || frontend.version != RUST_FRONTEND_VERSION
            || frontend.main.name != "rust2vir"
            || frontend.main.version != RUST_FRONTEND_VERSION
            || frontend.main.path != "bin/rust2vir"
            || frontend.main.binary_sha256 != RUST_FRONTEND_SHA256
            || !matches!(frontend.main.runtime, ExecutableRuntime::Dynamic { .. })
            || driver.name != "rust2vir-driver"
            || driver.version != RUST_FRONTEND_VERSION
            || driver.path != "bin/rust2vir-driver"
            || driver.binary_sha256 != RUST_DRIVER_SHA256
...
fn validate_toolchain(
    ...
    if toolchain.bundle_id == RUST_TOOLCHAIN_BUNDLE_ID {
        let names = toolchain
            .components
            .iter()
            .map(ToolchainComponent::name)
        ...
        if toolchain.execution_host_profile_id != RUST_HOST_PROFILE_ID
            || toolchain.distribution_sha256 != RUST_TOOLCHAIN_DISTRIBUTION_SHA256
            || names
                != [
                    "cargo",
                    "native-runtime",
...
fn validate_go_release_contract(
    ...
    let native_runtime = contract
        ...
        .and_then(Value::as_object)
        .ok_or_else(linkage_failure)?;
    if toolchain.bundle_id != GO_TOOLCHAIN_BUNDLE_ID
