...
fn projection_semantics(
    ...
    match profile {
        ...
        CompiledSemanticProfile::CSharpScalarV0 => Ok((
            SemanticProfile::GoFixedV0,
...
fn validate_projection(
    ...
    if profile != CompiledSemanticProfile::CSharpScalarV0 {
        return validate_vir(projection).map_err(|error| source_failure(error.to_string()));
    }

    // C# carries explicit checked/unchecked intent in its required-check list.
    // For the common structural/type validator only, map its canonical method
    // IDs to private Go-shaped surrogates and rewrite those lists to the Go
    // projection's canonical lists. The original IDs and lists are retained
    // and independently regenerated by the C# check profile in the WP pass.
    let mut structural = csharp_structural_projection(projection)?;
...
fn csharp_structural_projection(projection: &VirModule) -> Result<VirModule, SuccessorVcError> {
        ...
        || !projection.units[0].const_decls.is_empty()
    {
        return Err(source_failure(
            "C# scalar projection requires one unit and no type or constant declarations",
        ));
    }
    ...
    for (index, function) in structural.units[0].functions.iter().enumerate() {
        function_ids.insert(function.id.clone(), format!("csharp.f{index:04}"));
    }

    structural.units[0].id = "csharp".to_owned();
    structural.units[0].name = "csharp".to_owned();
    let mut contract_hashes = BTreeMap::new();
    for (index, function) in structural.units[0].functions.iter_mut().enumerate() {
        let id = function_ids
            .get(&function.id)
            .ok_or_else(|| source_failure("C# validation surrogate is absent"))?
            .clone();
        let name = format!("f{index:04}");
        function.id = id.clone();
        function.unit_id = "csharp".to_owned();
        function.name = name;
        function.contracts.unit_id = "csharp".to_owned();
            ...
            .flat_map(|block| &mut block.instructions)
        {
            ...
            } = instruction
            {
                *function = function_ids
                    .get(function)
                    .ok_or_else(|| source_failure("C# static callee surrogate is absent"))?
                    .clone();
                *contract_hash = contract_hashes
                    .get(function)
                    .ok_or_else(|| source_failure("C# static callee contract is absent"))?
                    .clone();
            }
        }
    }
