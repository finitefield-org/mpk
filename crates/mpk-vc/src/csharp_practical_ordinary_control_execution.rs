...
pub(super) fn append(
    ...
    for flow in &mut p.functions {
        ...
        for block in &native.blocks {
            ...
            execution_predicates(c, &mut operation)?;
            if let Some(owner) = ownership {
...
fn execution_predicates(
    c: &mut Clauses<'_>,
    operation: &mut OrdinaryControlNativeOperation,
) -> R<()> {
    let o = &operation.source;
...
/// Extend the original native declarations without replacing already-defined
/// relations. The same builder owns the retained recursive-relation caches.
pub(super) fn complete_options_and_structural(
    c: &mut Clauses<'_>,
    p: &mut OrdinaryControlEdgeProgram,
    vir: &ValidatedPracticalVir,
    foundations: &relations::DataFoundationState,
) -> R<()> {
    let missing = p
        .functions
        .iter()
        .flat_map(|f| &f.native_operations)
        .filter(|o| !o.pending_constant_names.is_empty())
        .map(|o| o.source.definition_id.as_str())
        .collect::<BTreeSet<_>>();
    if missing.is_empty() {
        return Ok(());
    }
    ...
    let definitions = data
        .definitions()
        .iter()
        .filter(|d| missing.contains(d.id.as_str()))
        .collect::<Vec<_>>();
    let option_operation = |d: &DataSemanticDefinition| {
        d.family == DataDefinitionFamily::Foundation
            && d.signature.id.rsplit_once('.').is_some_and(|(id, _)| {
                vir.data_closed().entries().iter().any(|e| {
                    e["instance_id"] == id && e["template_id"] == "mpk.csharp.semantic.option.v1"
                })
            })
    };
    let mut options = foundations.options.clone();
    if options.is_empty() && definitions.iter().any(|d| option_operation(d)) {
        let layouts = generate_csharp_practical_ordinary_carriers(vir)?;
        let builder = std::mem::replace(&mut c.b, Builder::new()?);
        let (builder, retained) = relations::emit_data_foundations_cached(
            vir,
            &layouts,
            builder,
            false,
            true,
            std::mem::take(&mut c.relations),
            std::mem::take(&mut c.storage),
        )?;
        c.b = builder;
        c.relations = retained.relations;
        c.storage = retained.storage;
        options = retained.options;
    }
    let mut completed = BTreeSet::new();
    for source in definitions {
        let definition = if source.family == DataDefinitionFamily::Structural {
            let d = structural_data::emit(c, source)?;
            entry(source, &d.relation_definition, std::iter::empty())
        } else if option_operation(source) {
            let operation = options
                .iter()
                .flat_map(|o| &o.operations)
                .find(|o| o.operation_id == source.signature.id)
                .ok_or(OrdinaryCarrierError::Linkage)?;
            let d = option_data::emit(c, source, operation)?;
            entry(
                source,
                &d.relation_definition,
                d.failure_definitions.into_iter().map(Some),
        ...
        if p.native_definitions
            .iter()
            .any(|d| d.source.id == source.id)
        {
            return Err(OrdinaryCarrierError::Linkage);
        }
        completed.insert(source.id.clone());
        p.native_definitions.push(definition);
    }
    for operation in p
        .functions
        .iter_mut()
        .flat_map(|f| &mut f.native_operations)
        .filter(|o| completed.contains(&o.source.definition_id))
    {
        if !operation.predicates.is_empty() || operation.ownership.is_some() {
            return Err(OrdinaryCarrierError::Linkage);
        }
        execution_predicates(c, operation)?;
    }
    Ok(())
}
