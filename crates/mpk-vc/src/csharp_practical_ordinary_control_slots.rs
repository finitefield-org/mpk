...
/// Emit source entry and successful transfers into the caller's owning builder.
/// The supplied memory bindings must come from this exact reconstructed flow.
pub(super) fn emit_function(
    b: &mut Builder,
    vir: &ValidatedPracticalVir,
    flow: &ControlFunctionVc,
    layouts: &OrdinaryCarrierProgram,
    memory_bindings: &[OrdinaryControlMemoryBinding],
) -> R<OrdinaryControlSlotFunction> {
    if flow.source_graph.is_none()
        || memory_bindings
            .iter()
            .any(|m| !flow.transfers.contains(&m.transfer))
    {
        return Err(OrdinaryCarrierError::Linkage);
    }
    let depths = layouts
    ...
    for transfer in &flow.transfers {
        ...
        let memory = memory_bindings.iter().find(|m| &m.transfer == transfer);
        relations.push(emit(
            b,
            &storage_flow,
            entry,
    ...
    Ok(OrdinaryControlSlotFunction {
        source: flow.clone(),
        relations,
        slot_type_overrides,
    })
...
pub fn generate_csharp_practical_ordinary_control_slots(
    ...
    for flow in control.functions() {
        ...
        let memory = memory_functions
            ...
            .map_or(&[][..], |f| f.memory_bindings.as_slice());
        functions.push(emit_function(&mut b, vir, flow, &layouts, memory)?);
    }
