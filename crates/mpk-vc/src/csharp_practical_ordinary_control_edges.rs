...
pub struct OrdinaryControlEdgeFunction {
    ...
    #[serde(skip_serializing_if = "Vec::is_empty")]
    pub slot_relations: Vec<OrdinaryControlSlotRelation>,
...
pub fn generate_csharp_practical_ordinary_control_edges(
    ...
    execution::complete_options_and_structural(&mut c, &mut p, vir, &foundations)?;
    for flow in &mut p.functions {
        if flow.source.source_graph.is_none() {
            continue;
        }
        let slots = control_slots::emit_function(
            &mut c.b,
            vir,
            &flow.source,
            &layouts,
...
pub(super) fn emit_program(
    ...
    let (builder, program, _) = emit_program_with_foundations(vir)?;
    Ok((builder, program))
}

fn emit_program_with_foundations(
    vir: &ValidatedPracticalVir,
) -> R<(
    Builder,
    OrdinaryControlEdgeProgram,
    relations::DataFoundationState,
)> {
    let data = crate::csharp_practical_vir_model::data_vc::generate_data_vcs(vir)
        .map_err(|_| OrdinaryCarrierError::Linkage)?;
    ...
    let mut options = vec![];
    let mut retained = relations::DataFoundationState::default();
    if !requested_sequences.is_empty()
        || !requested_references.is_empty()
        || !requested_options.is_empty()
    {
        (builder, retained) = relations::emit_data_foundations_cached(
            vir,
            &layouts,
            builder,
            !requested_sequences.is_empty(),
            !requested_references.is_empty() || !requested_options.is_empty(),
            relations::ContractRelationCache::default(),
            StorageCache::default(),
        )?;
        sequences = std::mem::take(&mut retained.sequences);
        options = std::mem::take(&mut retained.options);
    }
    let mut c = compiler(vir, &layouts, builder, &[])?;
    c.relations = std::mem::take(&mut retained.relations);
    c.storage = std::mem::take(&mut retained.storage);
    c.definedness_logic()?;
