...
fn csharp_03_t06_w09_decimal_fixed_formats_original_sources() {
    ...
    let root = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../develop/migrations/csharp-03/ordinary-foundation/decimal-fixed-formats");
    let mut rows = vec![];
    let mut tested = None;
    ...
    for (id, row, facts) in format_sources() {
        ...
        for field in [
            ...
            m[field] = json!("forged");
            assert!(import_csharp_practical_ordinary_decimal_fixed_formats(
        ...
        *bad.last_mut().unwrap() ^= 1;
        ...
        if let Some((m, c)) = &previous {
            assert!(import_csharp_practical_ordinary_decimal_fixed_formats(m, c, vir).is_err());
        }
        previous = Some((p.canonical_bytes(), p.certificate_bytes().to_vec()));
        if p.definitions().is_empty() {
            continue;
        }
        assert_eq!(p.definitions().len(), 145);
        let actual = p
            .definitions()
            .iter()
            .map(|d| (d.rounding.clone(), d.scale))
            .collect::<BTreeSet<_>>();
        let expected = [
            "ToEven",
            "AwayFromZero",
        ...
        .into_iter()
        .flat_map(|m| (0..=28).map(move |scale| (m.to_owned(), scale)))
        .collect::<BTreeSet<_>>();
        assert_eq!(actual, expected);
        assert_eq!(p.definitions()[0].codec_id, "decimal.fixed");
        let hex = p
            .certificate_bytes()
            .iter()
            .map(|b| format!("{b:02x}"))
    ...
    assert_eq!((examined, rows.len()), (65, 6));
    ...
    if let Some(dir) = &out {
        ...
        assert_eq!(
            read("ordinary-foundation/decimal-fixed-formats/certificates.json"),
            manifest
        );
    }
    ...
    for d in definitions {
        ...
        .unwrap();
        let mut inputs = BTreeSet::from([(false, 2u8, 125u128)]);
        if matches!(d.scale, 0 | 1 | 28) {
            inputs.extend([
                (true, 2, 125),
                (true, 28, 0),
                (false, 0, max),
        ...
        for (negative, scale, coefficient) in inputs {
            eprintln!("decimal fixed {} target {}: sign={negative}, scale={scale}, coefficient={coefficient}",d.rounding,d.scale);
    ...
    eprintln!("decimal fixed observations: {count}");
}

#[test]
fn csharp_03_t06_w09_decimal_fixed_formats_pinned_sources() {
    let bundle = b();
    let manifest = read("ordinary-foundation/decimal-fixed-formats/certificates.json");
    ...
    for (id, row, facts) in format_sources() {
        ...
        .unwrap();
        let emitted = emit_data_phase(&bundle, &context, &captures, &source).unwrap();
        let p = generate_csharp_practical_ordinary_decimal_fixed_formats(emitted.vir()).unwrap();
        if p.definitions().is_empty() {
