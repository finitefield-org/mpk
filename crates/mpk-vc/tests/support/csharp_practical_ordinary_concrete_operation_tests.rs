...
fn component_link(base: &Value, op: &OrdinaryConcreteOperationComponent) {
    ...
    for group in ["sequences", "entries", "transitions"] {
        ...
        if let Some(d) = rows.iter().find(|d| d["carrier"]["type_id"] == id) {
...
// A test-only closed IEEE word. Its five selectors are little-endian; the
// concrete observations below independently read every resulting bit.
fn word_literal(c: &mut Certificate, word: u32) -> u32 {
    fn push(c: &mut Certificate, t: TermNode) -> u32 {
        let id = c.term_table.len() as u32;
        c.term_table.push(t);
        id
    }
    let [boolean, no, yes, rec] = [
        "Std.Bool",
        "Std.Bool.false",
        "Std.Bool.true",
        "Std.Bool.rec",
    ...
    let mut terms = (0..32)
        .map(|i| if word & (1 << i) == 0 { no } else { yes })
        .collect::<Vec<_>>();
    for bit_index in (0..5).rev() {
        let selector = push(c, TermNode::Var(4 - bit_index));
        ...
        terms = (0..half)
            .map(|i| {
                push(
                    ...
                    TermNode::App {
                        function: rec,
                        arguments: vec![terms[i], terms[i + half], selector],
                    },
                )
            })
            .collect();
...
#[test]
fn csharp_03_t06_w09_concrete_operations_isolated_payload_and_invalid_input() {
    let bundle = b();
    let (mut payload_changes, mut invalid_inputs) = (0, 0);
    for (id, row, facts) in sources()
        .into_iter()
        .filter(|(id, _, _)| ["binding-vc-option", "float-make-commutation"].contains(&id.as_str()))
    {
        let (context, captures) = support::replay_context(&bundle, &row);
        let source = ValidatedDataSource::import_captured_facts(
            &bundle,
        ...
        if id == "float-make-commutation" {
            let op = p
                .definitions()
                .iter()
                .find(|d| d.component.operation_id.ends_with(".make"))
                .unwrap();
            let condition = p
                .conditions()
                .iter()
            ...
            let mut changed = cert.clone();
            let literal = word_literal(&mut changed, 0x7fc0_1235);
            let mut old_body = body(&cert, &op.concrete_definition);
            let mut types = vec![];
            for _ in 0..2 {
                let TermNode::Lam { ty, body } = cert.term_table[old_body as usize] else {
                    panic!()
                };
                types.push(ty);
                old_body = body;
            }
            let function = changed.term_table.len() as u32;
            changed.term_table.push(TermNode::Const {
                global: global(&cert, &op.component.normal_definition),
                levels: vec![],
            });
            let key = changed.term_table.len() as u32;
            changed.term_table.push(TermNode::Var(1));
            let mut value = changed.term_table.len() as u32;
            changed.term_table.push(TermNode::App {
                function,
                arguments: vec![key, literal],
            });
            for ty in types.into_iter().rev() {
                let body = value;
                value = changed.term_table.len() as u32;
                changed.term_table.push(TermNode::Lam { ty, body });
            }
            let DeclarationKind::Def {
                value: previous, ..
            } = &mut changed.declarations[global(&cert, &op.concrete_definition) as usize].kind
            else {
                panic!()
            };
            *previous = value;
            let key = p
                .definitions()
                .iter()
                .find(|d| d.component.operation_id.ends_with(".key"))
                .unwrap();
            let field = p
                .definitions()
                .iter()
                .find(|d| d.component.operation_id.ends_with(".value"))
                .unwrap();
            for (name, expected) in [
                (&op.normal_definition, 0x7fc0_1234u32),
                (&op.concrete_definition, 0x7fc0_1235u32),
            ] {
                let value = run(&changed, name, args.clone());
                assert!(!bit(run(
                    &changed,
                    &key.normal_definition,
                    vec![value.clone()]
                )));
                let field = run(&changed, &field.normal_definition, vec![value]);
                for i in 0..32 {
                    let observed = (0..5).fold(field.clone(), |f, n| {
                        core_eval::apply(&changed, f, V::Bit(i & (1 << n) != 0))
                    });
                    assert_eq!(bit(observed), expected & (1 << i) != 0);
                }
            }
            assert!(bit(run(
                &cert,
            ...
            for name in [
                &op.normal_agreement_definition,
                &op.all_outcomes_definition,
                &condition.condition_definition,
            ] {
                assert!(
                    !bit(run(&changed, name, args.clone())),
                    "isolated NaN payload change ignored"
                );
            }
            payload_changes += 1;
        } else {
            let op = p
                .definitions()
                .iter()
                .find(|d| d.component.operation_id.ends_with(".has_value"))
                .unwrap();
            let condition = p
                .conditions()
                .iter()
            ...
            let domain = p
                .public_domains()
                .iter()
                .find(|d| d.carrier.type_id == op.component.argument_type_ids[0])
                .unwrap();
            // An unknown tag violates the exact public Option representation.
            // The total has_value operation still evaluates to false.
            let args = vec![V::UniformCube(true, domain.carrier.depth)];
            assert!(!bit(run(&cert, &domain.valid_definition, args.clone())));
            let mut changed = cert.clone();
            constant_result(&mut changed, &op.concrete_definition, 1, true);
            assert!(!bit(run(
                &changed,
                &op.normal_agreement_definition,
                args.clone()
            )));
            assert!(!condition.goal_definitions.iter().all(|name| bit(run(
                &changed,
                name,
                args.clone()
            ))));
            assert!(!condition.assumption_definitions.iter().all(|name| bit(run(
                &changed,
                name,
                args.clone()
            ))));
            assert!(bit(run(&changed, &condition.condition_definition, args)));
            invalid_inputs += 1;
        }
    }
    assert_eq!((payload_changes, invalid_inputs), (1, 1));
    eprintln!("isolated concrete operations: {payload_changes} NaN payload changes, {invalid_inputs} malformed source inputs");
}
