#!/usr/bin/env python3
"""Build and validate Java roots for the sole active successor release."""
...
SPEC = importlib.util.spec_from_file_location(
    "java_build_owner", Path(__file__).with_name("java_build_inputs.py")
)
...
ACTIVE_CANDIDATE = "release/bundles/candidates/java.json"
ARCHIVED_CANDIDATE = "release/build-inputs/java/bundle-candidate.json"
    ...
    for name, row in sorted(entries.items()):
        ...
        while entries[target]["kind"] == "symlink":
            ...
            target = posixpath.normpath(
                posixpath.join(posixpath.dirname(target), entries[target]["target"])
            )
            BUILD.require(
                target in entries and not target.startswith("../"),
                "JAVA_CANDIDATE_LINK",
            )
        ...
        result.append(
            (
                dict(
                    path="jdk/" + name,
                    executable=bool(int(source["mode"], 8) & 0o111),
...
def descriptors():
    ...
    semantic = BUILD.load_json(
        BUILD.ROOT / "develop/specs/vectors/semantic-profile-registry-v3.json"
    )["registry"]
    ...
    notice = next(
        row for row in descriptor["project_files"] if row["path"] == "NOTICE.txt"
    )
    ...
    frontend = dict(
        ...
        main=dict(
            ...
            version="0.1.0",
            path="java2vir.jar",
            binary_sha256=measured["frontend_files"][0]["sha256"],
            runtime=dict(kind="static"),
        ),
        subordinate_binaries=[],
        inventory=frontend_inventory,
        bundle_sha256=BUILD.sha256(
            CONTENT_DOMAIN + BUILD.canonical(frontend_inventory)
        ),
    )
    jdk = [row for row, _ in jdk_file_records(inputs)]
    native = [
        dict(
            path="native-runtime/" + row["path"],
            executable=bool(int(row["mode"], 8) & 0o111),
    ...
    runtime = dict(
        kind="dynamic",
        interpreter_mount=inputs["host"]["interpreter"],
        libraries=[
            dict(
                soname=Path(row["path"]).name,
                component_path=row["path"],
                sha256=row["sha256"],
            )
            for row in sorted(
                inputs["native_inventory"], key=lambda row: Path(row["path"]).name
            )
    ...
    toolchain = dict(
        ...
        execution_host_profile_id=HOST_ID,
        profile_contracts=[contracts["release"]],
        components=components,
        inventory=toolchain_inventory,
        distribution_sha256=BUILD.sha256(
            CONTENT_DOMAIN + BUILD.canonical(toolchain_inventory)
        ),
    )
    host = dict(
        id=HOST_ID,
        ...
        probe_profile_id="mpk.release.probe.java25.v0",
        required_primitives=sorted(
            [
                "cgroup2.atomic_clone3",
                "cgroup2.descendant_cleanup",
                "cgroup2.memory_1073741824",
                "cgroup2.pids_128",
                "cgroup2.swap_0",
                "fs.readonly_inputs",
                "fs.tmpfs_67108864_noswap",
                "linux.namespaces",
                "process.closed_environment",
                "process.no_new_privileges",
                "process.no_network",
                "process.nonroot_no_capabilities",
                "process.rlimit_as_17179869184",
                "process.rlimit_core_0",
                "process.rlimit_nofile_1024",
                "process.seccomp_java_threads",
                "process.timeout_120",
                "proc.readonly_private_pid_namespace",
            ]
        ),
    )
    layout = dict(
...
def build_roots(work, expected_candidate):
    """Build the two immutable Java roots for the composed successor image."""
    candidate = descriptors()
    BUILD.require(candidate == expected_candidate, "JAVA_CANDIDATE_DESCRIPTOR")
    inputs, descriptor = BUILD.load_toolchain(), BUILD.load_descriptor()
    measured, jar = BUILD.build_twice(inputs, descriptor)
    BUILD.require(
        measured == BUILD.load_inventory(descriptor), "JAVA_CANDIDATE_DESCRIPTOR"
    )
    ...
    put(
        frontend / "NOTICE.txt",
        BUILD.read_bytes(BUILD.ROOT / BUILD.PROJECT / "NOTICE.txt", BUILD.MAX_SOURCE),
    )
    for row, target in jdk_file_records(inputs):
        data = BUILD.read_bytes(jdk / target, row["size_bytes"])
        BUILD.require(
            len(data) == row["size_bytes"] and BUILD.sha256(data) == row["sha256"],
            "JAVA_CANDIDATE_JDK",
        )
    ...
    for root in (frontend, toolchain):
        for directory, _, _ in os.walk(root, topdown=False):
            Path(directory).chmod(0o555)
    return {FRONTEND_ID: frontend, TOOLCHAIN_ID: toolchain}


def native_bytes(inputs):
    """Read only the frozen native files from the provisioned image."""
...
"""
    with tempfile.TemporaryDirectory(prefix="mpk-java-native-", dir="/tmp") as directory:
        ...
        try:
            ...
            report = BUILD.strict_json(
                stdout, maximum=16 * 1024 * 1024, canonical_transport=True
            )
            BUILD.require(
                set(report) == {row["path"] for row in inputs["native_inventory"]},
                "JAVA_CANDIDATE_NATIVE",
            )
            result = {}
            for row in inputs["native_inventory"]:
                data = base64.b64decode(report[row["path"]], validate=True)
                BUILD.require(
                    len(data) == row["bytes"] and BUILD.sha256(data) == row["sha256"],
                    "JAVA_CANDIDATE_NATIVE",
                )
...
def main(arguments):
    BUILD.require(arguments == ["check"], "JAVA_CANDIDATE_USAGE", 64)
    expected = descriptors()
    active = BUILD.load_json(BUILD.ROOT / ACTIVE_CANDIDATE, canonical_transport=True)
    archived = BUILD.load_json(BUILD.ROOT / ARCHIVED_CANDIDATE, canonical_transport=True)
    BUILD.require(active == expected == archived, "JAVA_CANDIDATE_DESCRIPTOR")
...
if __name__ == "__main__":
    try:
        ...
        print(
            error.code if isinstance(error, BUILD.BuildFailure) else "JAVA_CANDIDATE_INVALID",
            file=sys.stderr,
        )
