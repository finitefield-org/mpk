...
./...` and lower through `go2gir` without rejected features.

`go-tools/go2gir` tests execute every entry in `manifest.json`, verify the
manifested function count, and require all 100 functions to lower to GIR.
